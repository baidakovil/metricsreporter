...
- Metrics artifacts accessible (OpenCover XML, Roslyn metrics XML, SARIF files, baseline).
...
Scripts run in order; the pipeline stops on the first non-zero exit code. The repo’s default script delegates build/metrics collection to the `GenerateSolutionMetrics` MSBuild target, then the CLI aggregates inputs.
...
Notes:
- CLI overrides config if provided (e.g., `--opencover tmp/cov.xml`).
