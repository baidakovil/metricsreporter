...
- Console: always on, single-line UTC timestamps.
- File sink: enabled when you pass `--log-path` (CLI) or set `paths.logPath` in `.metricsreporter.json`. If omitted, the tool writes `MetricsReporter.log` beside the report/baseline.
...
- Generate with explicit log path and detailed verbosity:
  - `metricsreporter generate --metrics-dir build/Metrics --altcover build/Metrics/coverage.xml --roslyn build/Metrics/metrics.xml --output-json build/Metrics/Report/metrics-report.json --log-path build/Metrics/Report/MetricsReporter.log --verbosity detailed`
