...
- You are iterating on CLI output (e.g., `read` command formatting) and need fast feedback.
...
- Working copy of the repo (cloned with `git clone https://github.com/baidakovil/metricsreporter.git`).
- .NET 8 SDK.
- Input artifacts (OpenCover XML, Roslyn metrics, SARIF, baseline) already available or referenced via CLI flags.

### Step 1: Build the tool project (optional if you use the local tool manifest)
...
This produces `MetricsReporter.Tool/bin/Debug/net8.0/MetricsReporter.Tool.dll` for direct `dotnet run`. You can also rely on `dotnet tool run metricsreporter` (from the repo manifest) without rebuilding if packages are current.
...
dotnet run --project MetricsReporter.Tool/MetricsReporter.Tool.csproj -- `
  read `
  --report C:\work\Metrics\MetricsReport.g.json `
...
```powershell
dotnet tool run metricsreporter read `
  --namespace Sample.Loader `
  --metric Coupling
```
...
dotnet run --project MetricsReporter/MetricsReporter.csproj -- `
  ...
  --metrics-dir "C:\\work\\Metrics" `
  --opencover "C:\\work\\Metrics\\OpenCover\\coverage-runtime.xml" `
  --roslyn "C:\\work\\Metrics\\Roslyn\\Sample.Loader.xml" `
  --sarif "C:\\work\\Metrics\\Sarif\\Sample.Loader.sarif" `
  --baseline "C:\\work\\Metrics\\MetricsTemp\\MetricsBaseline.g.json" `
  --output-json "C:\\work\\Metrics\\MetricsReport.g.json" `
  --output-html "C:\\work\\Metrics\\MetricsReport.html"
```
...
Because `dotnet run` uses the compiled assemblies inside `bin/Debug`, you can just re-run `dotnet build` after each code change—no extra packaging or tool installation is required.
