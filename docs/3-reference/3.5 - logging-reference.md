...
- CLI composition roots register logging with `ServiceCollection` and use `LoggerFactoryBuilder.Create(logPath, minimumLevel)` to add a console logger plus an optional file sink.
...
- Default console output uses single-line messages with UTC timestamps.
