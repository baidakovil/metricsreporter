...
- For AltCover metrics, prefer suppression so coverage values stay in the report (avoid attribute-based exclusion that removes rows).
...
- Metrics Reporter normalizes:
  ...
  - Uses `RuleId` → metric mapping (`CA1506` → `RoslynClassCoupling`; `AltCoverBranchCoverage` stays as-is).
...
- Choose the right category and rule id:
  - AltCover: `AltCoverBranchCoverage`, `AltCoverSequenceCoverage`.
