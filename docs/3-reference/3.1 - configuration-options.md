...
| `paths` | `metricsDir`, `solutionName`, `baselineReference`, `report` (generate output JSON), `readReport` (input JSON for read/readsarif/test), `thresholds`, `thresholdsInline`, arrays `altcover[]`, `roslyn[]`, `sarif[]`, `sourceCodeFolders[]`, plus `baseline`, `outputHtml`, `inputJson`, `coverageHtmlDir`, `baselineStoragePath`, `suppressedSymbols`, `solutionDirectory`, `excludedMembers`, `excludedAssemblies`, `excludedTypes`, `excludeMethods`, `excludeProperties`, `excludeFields`, `excludeEvents`, `analyzeSuppressedSymbols` (bool), `replaceBaseline` (bool) | Paths are required when the corresponding command needs them; missing required paths cause validation errors. |
...
| `metricAliases` | Map `MetricIdentifier` ➜ array of aliases | Optional. Keys must be canonical identifiers (e.g. `AltCoverBranchCoverage`), values are arrays of non-empty strings. There are no built-in aliases — declare all friendly names here or via env/CLI. |
...
- Aliases: `METRICSREPORTER_METRIC_ALIASES` as JSON, e.g. `{"AltCoverBranchCoverage":["branch","branchcoverage"]}`. When absent, only canonical identifiers are accepted.
...
```json
{
  ...
  "metricAliases": {
    "AltCoverSequenceCoverage": [ "SequenceCoverage", "Coverage" ],
    "AltCoverBranchCoverage": [ "BranchCoverage" ],
    "AltCoverCyclomaticComplexity": [ "AltCoverComplexity" ],
    "AltCoverNPathComplexity": [ "NPath" ],
