...
| Section | Keys | Notes |
| --- | --- | --- |
| `general` | `runScripts`, `aggregateAfterScripts`, `verbosity`, `timeoutSeconds`, `workingDirectory`, `logTruncationLimit`, `editorPrefix` | `runScripts` and `aggregateAfterScripts` default to `true`. `verbosity` accepts `quiet|minimal|normal|detailed`. `timeoutSeconds` and `logTruncationLimit` must be > 0. `editorPrefix` controls the URL scheme for source links in the HTML report (e.g., `vscode://file/`, `cursor://`). Defaults to `vscode://file/`. |
| `paths` | `metricsDir`, `solutionName`, `baselineReference`, `report` (generate output JSON), `readReport` (input JSON for read/readsarif/test), `thresholds`, `thresholdsInline`, arrays `opencover[]`, `roslyn[]`, `sarif[]`, `sourceCodeFolders[]`, plus `baseline`, `outputHtml`, `inputJson`, `coverageHtmlDir`, `baselineStoragePath`, `suppressedSymbols`, `solutionDirectory`, `excludedMembers`, `excludedAssemblies`, `excludedTypes`, `excludeMethods`, `excludeProperties`, `excludeFields`, `excludeEvents`, `analyzeSuppressedSymbols` (bool), `replaceBaseline` (bool) | Paths are required when the corresponding command needs them; missing required paths cause validation errors. |
| `scripts` | `generate[]`, `read.any[]`, `read.byMetric[]`, `test.any[]`, `test.byMetric[]` | PowerShell script paths. `byMetric` items are `{ "metrics": ["RoslynClassCoupling", "OpenCoverBranchCoverage"], "path": "tools/run-before-read.ps1" }`. `scripts.test` is used only by the `test` command. |
| `metricAliases` | Map `MetricIdentifier` ➜ array of aliases | Optional. Keys must be canonical identifiers (e.g. `OpenCoverBranchCoverage`), values are arrays of non-empty strings. There are no built-in aliases — declare all friendly names here or via env/CLI. |
...
- Paths: `METRICSREPORTER_PATHS_METRICS_DIR`, `..._SOLUTION_NAME`, `..._BASELINE_REF`, `..._REPORT`, `..._READ_REPORT`, `..._THRESHOLDS`, `..._THRESHOLDS_INLINE`, `..._OPENCOVER`, `..._ROSLYN`, `..._SARIF`, `..._BASELINE`, `..._OUTPUT_HTML`, `..._INPUT_JSON`, `..._COVERAGE_HTML_DIR`, `..._BASELINE_STORAGE_PATH`, `..._SUPPRESSED_SYMBOLS`, `..._SOLUTION_DIRECTORY`, `..._SOURCE_CODE_FOLDERS`, `..._EXCLUDED_MEMBERS`, `..._EXCLUDED_ASSEMBLIES`, `..._EXCLUDED_TYPES`, `..._ANALYZE_SUPPRESSED_SYMBOLS`, `..._REPLACE_BASELINE`.
...
- Aliases: `METRICSREPORTER_METRIC_ALIASES` as JSON, e.g. `{"OpenCoverBranchCoverage":["branch","branchcoverage"]}`. When absent, only canonical identifiers are accepted.
...
- `metricAliases` keys must be valid `MetricIdentifier` names; values must be non-empty string arrays. Aliases are trimmed, empty strings are rejected, and the same alias cannot map to different metrics.

### Conditional required settings and warnings
- Дополнительно к валидации, CLI логирует предупреждения, если включены опции без обязательных значений (например, `replaceBaseline=true` без `paths.baseline`). Правила и сообщения описаны в JSON-матрице `Configuration/Warnings/required-settings.matrix.json` (схема в `Configuration/Warnings/required-settings.schema.json`).
...
| `--opencover <path>` | appended to `paths.opencover[]` |
...
```json
{
  ...
  "paths": {
    ...
    "opencover": [ "Metrics/MetricsTemp/CoverageStorage.g.xml" ],
    "suppressedSymbols": "Metrics/MetricsTemp/SuppressedSymbols.g.json",
    "baselineStoragePath": "%LOCALAPPDATA%/MetricsReporter/Metrics",
  ...
  "scripts": {
    ...
    "read": {
      ...
      "byMetric": [
        { "metrics": [ "OpenCoverBranchCoverage", "OpenCoverSequenceCoverage" ], "path": "scripts/coverage.ps1" },
  ...
  "metricAliases": {
    "OpenCoverSequenceCoverage": [ "SequenceCoverage", "Coverage" ],
    "OpenCoverBranchCoverage": [ "BranchCoverage" ],
    "OpenCoverCyclomaticComplexity": [ "OpenCoverComplexity" ],
    "OpenCoverNPathComplexity": [ "NPath" ],
    "RoslynMaintainabilityIndex": [ "Maintainability", "MaintainabilityIndex" ],
    "RoslynCyclomaticComplexity": [ "Complexity", "CyclomaticComplexity", "cc", "cyclomatic" ],
    "RoslynClassCoupling": [ "Coupling", "ClassCoupling" ],
    "RoslynDepthOfInheritance": [ "Inheritance", "DepthOfInheritance" ],
    "RoslynSourceLines": [ "SourceLines" ],
    "RoslynExecutableLines": [ "ExecutableLines" ],
    "SarifCaRuleViolations": [ "CaViolations" ],
    "SarifIdeRuleViolations": [ "IdeViolations" ]
  }
}
```
