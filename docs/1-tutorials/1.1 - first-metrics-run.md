...
- OpenCover instrumentation targets referenced by the consumer solution (already wired into `code-metrics.props`).
...
```powershell
# Build or CI steps should produce coverage/metrics inputs first (OpenCover, Roslyn, SARIF).
...
1. Your prior steps (or `scripts.generate` hooks) must produce OpenCover XML, Roslyn metrics XML, and SARIF files under `build/Metrics`.
...
- Add more OpenCover files by repeating `/p:OpenCoverReports=...` or CLI `--opencover` arguments; the aggregator fails fast if a symbol appears in multiple files, protecting data integrity.
