...
| `fullyQualifiedName` | string? | Present for `Assembly`, `Type`, `Member`. Uses dot notation with `(...)` placeholder for method parameters. |
...
- `src/MetricsReporter/Model/metrics-report.schema.json` – authoritative JSON schema for reports and baselines.
- `src/MetricsReporter/Model/suppressed-symbols.schema.json` – schema for `SuppressedSymbols.g.json`.
