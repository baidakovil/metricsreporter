...
This page summarizes the configuration model enforced by `metricsreporter` and the schema in `src/MetricsReporter/Configuration/metricsreporter-config.schema.json`. Use it to validate pipeline configs and to know which source wins when values differ.
...
4. **Defaults** – `verbosity=normal`, `timeoutSeconds=900`, `logTruncationLimit=4000`, `workingDirectory=current directory`.
...
| Section | Keys | Notes |
| --- | --- | --- |
| `general` | `verbosity`, `timeoutSeconds`, `workingDirectory`, `logTruncationLimit` | `verbosity` accepts `quiet|minimal|normal|detailed`. `timeoutSeconds` and `logTruncationLimit` must be > 0. |
...
| `scripts` | `generate[]`, `read.any[]`, `read.byMetric[]` | PowerShell script paths. `read.byMetric` items are `{ "metrics": ["RoslynClassCoupling", "AltCoverBranchCoverage"], "path": "tools/run-before-read.ps1" }`. |
...
- Scripts: `METRICSREPORTER_SCRIPTS_GENERATE`, `METRICSREPORTER_SCRIPTS_READ_ANY`, `METRICSREPORTER_SCRIPTS_READ_BYMETRIC` (semicolon-separated `metric1,metric2:path.ps1` entries).
...
Readers (`read`, `readsarif`, `test`) use `paths.readReport` as the default report input and the `scripts.read.*` hooks. Metric-scoped scripts run when any requested metric matches one of the configured identifiers.
...
### Sample config (paths from `build/Props/paths.props`)
```json
{
  "general": {
    "verbosity": "normal",
