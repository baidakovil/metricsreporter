...
dotnet pack src/MetricsReporter/MetricsReporter.csproj -c Release `
  -o C:\Users\baidakov\.nuget\local-feed
dotnet pack src/MetricsReporter.Tool/MetricsReporter.Tool.csproj -c Release `
  -o C:\Users\baidakov\.nuget\local-feed
```
...
3. Optionally run the MSBuild pipeline (`dotnet build --no-incremental /p:GenerateMetricsDashboard=true ...`) to prove the packaged bits generate HTML correctly.
