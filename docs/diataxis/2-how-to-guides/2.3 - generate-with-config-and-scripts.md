...
### 2) Add prep script(s)
Place `scripts/prepare-metrics.ps1` (example):
...
Scripts run in order; the pipeline stops on the first non-zero exit code.
...
- Logs: `MetricsReporter.log` is written under `build/Metrics/Report` (or next to `--output-json`).

### 4) Validate outputs
- `build/Metrics/Report/MetricsReport.g.json` must exist.
- `build/Metrics/Report/MetricsReport.html` is produced when `--output-html`/`paths.outputHtml` is set.
