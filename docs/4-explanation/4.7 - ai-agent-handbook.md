...
- `MetricsReporter` – Library that aggregates metrics and emits JSON/HTML.
- `MetricsReporter.Tool` – Pack-as-tool CLI exposing `metricsreporter` commands (Spectre.Console.Cli).
- `MetricsReporter.Tests` – NUnit + FluentAssertions + NSubstitute.
- `docs/` – Diataxis-structured documentation (tutorials / how-to / reference / explanation).

### Distribution model

- The tool is published to NuGet.org as `MetricsReporter.Tool`.
- Consumers install it with `dotnet tool install --global MetricsReporter.Tool` or pin a version in `.config/dotnet-tools.json`.
- Tool binaries live in `%USERPROFILE%\.dotnet\tools` after global install.
- The library can also be referenced directly via `PackageReference MetricsReporter`.
...
- Use `dotnet run --project MetricsReporter.Tool/MetricsReporter.Tool.csproj -- <command>` when you need instant feedback without packing.
- For generator experiments, target `MetricsReporter/MetricsReporter.csproj` directly and pass explicit `--opencover`, `--roslyn`, `--sarif`, `--baseline`, `--output-json`, and `--output-html` flags.
...
```powershell
# prepare inputs in scripts/generate*.ps1 if needed (OpenCover, Roslyn, SARIF, baselines)
metricsreporter generate `
  --metrics-dir build/Metrics `
  --opencover build/MetricsTemp/CoverageStorage.g.xml `
  ...
  --coverage-html-dir build/Metrics/OpenCover/html
```
