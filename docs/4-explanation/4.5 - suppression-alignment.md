...
- `NamespaceMatcher` (used by the `read` command) treats `.`, `+`, and `:` as acceptable separators immediately after the namespace prefix. Queries such as `metricsreporter read --namespace MetricsReporter --symbol-kind Type` therefore match both `MetricsReporter.Aggregation.StructuralElementMerger` and any nested types (`MetricsReporter.Aggregation.StructuralElementMerger.MemberResolutionContext`).
